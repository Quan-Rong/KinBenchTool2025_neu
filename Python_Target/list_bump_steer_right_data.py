"""列出Bump Steer Right的样本点数据

从res文件中提取ID 776 (wheel_travel.vertical_right) 和 ID 881 (toe_angle.right)，
并显示用于拟合的样本点。
"""
...
def list_bump_steer_right_data(res_file_path: str, fit_range: int = 15):
    """列出Bump Steer Right的样本点数据
    ...
    print(f"\n参数ID:")
    print(f"  Toe Angle Right (ID 881): {toe_angle_ID[1]}")
    print(f"  Wheel Travel Right (ID 776): {wheel_travel_ID[1]}")
    print()
    
    # 从quasi数据中提取原始数据
    # ID 881: toe_angle.right (原始单位: rad)
    toe_angle_right_raw = quasi_data[:, toe_angle_ID[1]]  # 单位: rad
    
    # ID 776: wheel_travel.vertical_right (原始单位: mm)
    wheel_travel_right_raw = quasi_data[:, wheel_travel_ID[1]]  # 单位: mm
    
    # 单位换算：toe_angle从rad转为deg
    toe_angle_right_deg = convert_angle_array(toe_angle_right_raw, from_unit='rad', to_unit='deg')
    
    print(f"数据点总数: {len(toe_angle_right_deg)}")
    print()
    ...
    print(f"零位置对应的轮跳: {wheel_travel_right_raw[Row_No]:.3f} mm")
    print(f"零位置对应的Toe角: {toe_angle_right_deg[Row_No]:.6f} deg")
    print()
    
    # 计算拟合区间
    zero_idx = Row_No
    fit_start = max(0, zero_idx - fit_range)
    fit_end = min(len(toe_angle_right_deg), zero_idx + fit_range + 1)
    
    print(f"拟合区间: [{fit_start}, {fit_end})")
    print(f"拟合区间范围: {fit_end - fit_start} 个点")
    print(f"拟合区间对应的轮跳范围: [{wheel_travel_right_raw[fit_start]:.3f}, {wheel_travel_right_raw[fit_end-1]:.3f}] mm")
    print()
    print("=" * 80)
    print("用于拟合的样本点数据 (Bump Steer Right):")
    print("=" * 80)
    print(f"{'索引':<8} {'X轴 (ID 776)':<20} {'Y轴 (ID 881)':<20} {'Y轴原始(rad)':<20}")
    print(f"{'':<8} {'Wheel Travel [mm]':<20} {'Toe Angle [deg]':<20} {'Toe Angle [rad]':<20}")
    print("-" * 80)
    
    # 显示拟合区间内的所有样本点
    for idx in range(fit_start, fit_end):
        x_val = wheel_travel_right_raw[idx]  # mm
        y_val_deg = toe_angle_right_deg[idx]  # deg
        y_val_rad = toe_angle_right_raw[idx]  # rad
    ...
    # 显示统计信息
    x_fit = wheel_travel_right_raw[fit_start:fit_end]
    y_fit = toe_angle_right_deg[fit_start:fit_end]
    ...
    return {
        'x_data': x_fit,
        'y_data': y_fit,
        'x_data_raw': wheel_travel_right_raw[fit_start:fit_end],
        'y_data_raw_rad': toe_angle_right_raw[fit_start:fit_end],
        'fit_start': fit_start,
        'fit_end': fit_end,
        'row_no': Row_No,
        'coeffs': coeffs,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="列出Bump Steer Right的样本点数据")
    ...
    try:
        list_bump_steer_right_data(args.res_file, args.fit_range)
